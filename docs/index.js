...
const{after,instead,before}=findByProps("after","before","instead")||{}
...
if(!selfId)return
FluxDispatcher.dispatch({type:"CURRENT_USER_UPDATE",user:{...findByStoreName("UserStore").getCurrentUser()}})
FluxDispatcher.dispatch({type:"USER_PROFILE_UPDATE",userProfile:{...findByProps("getUserProfile","getGuildMemberProfile").getUserProfile(selfId)}})
}
let patches=[]

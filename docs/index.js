...
h(FormInput,{title:"Fake Join Year",placeholder:"e.g. 2020",value:jy,onChange:function(v){setJy(v);s.joinYear=v;refreshUser();setTimeout(refreshProfile,100)}})),
...
const r=s.enabled&&s.joinYear&&selfId&&sn===selfId
if(r)vendetta.logger.log("[LarpPlugin] extractTimestamp faked for",sn)
return r?new Date(parseInt(s.joinYear),0,1).getTime():origExt.apply(this,arguments)
}
patches.push(function(){SnowflakeUtils.extractTimestamp=origExt})
}
const usr=findByStoreName("UserStore")?.getCurrentUser?.()
vendetta.logger.log("[LarpPlugin] user keys:",Object.keys(usr||{}).join(","))
refreshUser()

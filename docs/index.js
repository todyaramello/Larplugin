...
function applyFakes(user){if(!user||!s.enabled)return user;if(s.username)user.username=s.username;if(s.displayName)user.globalName=s.displayName;if(s.email)user.email=s.email;if(s.phone)user.phone=s.phone;if(s.bio)user.bio=s.bio;if(s.avatar)user.avatar=s.avatar;if(s.banner)user.banner=s.banner;if(s.avatarDecoration)user.avatarDecoration=s.avatarDecoration;if(s.joinYear){const d=new Date(parseInt(s.joinYear),Math.floor(Math.random()*12),Math.floor(Math.random()*28)+1).getTime();user.createdAt=d;user.timestamp=d};if(s.accent){const n=parseInt(s.accent.replace("#",""),16);if(!isNaN(n))user.accentColor=n};if(s.accent2){const n=parseInt(s.accent2.replace("#",""),16);if(!isNaN(n))user.accentColor2=n};const flags=calculateFlags();if(flags>0){user.flags=flags;user.publicFlags=flags};return user}

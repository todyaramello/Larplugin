...
const CUSTOM_HASH="a_larp_custom_0000000000000"
function applyFakes(user){if(!user||!s.enabled)return user;if(s.avatar)user.avatar=CUSTOM_HASH;if(s.banner)user.banner=CUSTOM_HASH;if(s.username)user.username=s.username;if(s.displayName)user.globalName=s.displayName;if(s.email)user.email=s.email;if(s.phone)user.phone=s.phone;if(s.bio)user.bio=s.bio;if(s.joinYear){const d=new Date(parseInt(s.joinYear),Math.floor(Math.random()*12),Math.floor(Math.random()*28)+1).getTime();user.createdAt=d;user.timestamp=d};if(s.accent){const n=parseInt(s.accent.replace("#",""),16);if(!isNaN(n))user.accentColor=n};const flags=calculateFlags();if(flags>0){user.flags=flags;user.publicFlags=flags};return user}
...
if(s.joinYear){const d=new Date(parseInt(s.joinYear),Math.floor(Math.random()*12),Math.floor(Math.random()*28)+1).getTime();clone.joinedAt=d;clone.createdAt=d};if(s.accent||s.accent2){const n1=parseInt((s.accent||"#5865F2").replace("#",""),16);const n2=parseInt((s.accent2||s.accent||"#5865F2").replace("#",""),16);if(!isNaN(n1)&&!isNaN(n2)){clone.accentColor=n1;clone.themeColors=[n1,n2]}}FluxDispatcher.dispatch({type:"USER_PROFILE_UPDATE",userProfile:clone})
}
function pickImage(cb){
let I;try{I=findByProps("launchImageLibrary")}catch(e){}
if(I&&I.launchImageLibrary){
I.launchImageLibrary({mediaType:"photo",quality:1,includeBase64:true},function(r){
if(r.assets&&r.assets[0]?.base64){
const uri="data:image/jpeg;base64,"+r.assets[0].base64
cb(uri)
}else if(r.assets&&r.assets[0]?.uri){
cb(r.assets[0].uri)
}else console.warn("[LarpPlugin] No image data")
})
}else{console.warn("[LarpPlugin] No image picker")}
}
function patchURL(key){
return function(){
const cu=findByStoreName("UserStore")?.getCurrentUser?.()
if(s.enabled&&cu&&s[key])return s[key]
const a=arguments,f=a[0],id=f&&typeof f==="object"?f.id:f
if(s.enabled&&cu&&id===cu.id)return s[key]
const orig=this&&this[key]?this[key]:function(){}
...
const selected=c.toUpperCase()===(current||"#5865F2").toUpperCase()
return h(RN.TouchableOpacity,{key:c,style:{width:36,height:36,borderRadius:18,backgroundColor:c,borderWidth:selected?3:0,borderColor:"#fff"},onPress:function(){onPick(c)}})
}))
}
function Settings(){
...
h(ColorRow,{colors:COLORS,current:ac1,onPick:function(v){setAc1(v);s.accent=v;refreshUser();setTimeout(refreshProfile,100)}}),
h(FormInput,{title:"",placeholder:"e.g. #FF0000",value:ac1,onChange:function(v){setAc1(v);s.accent=v;refreshUser();setTimeout(refreshProfile,100)}}),
h(RN.Text,{style:{color:"#ccc",marginTop:8,marginBottom:4}},"Color 2 (right)"),
h(ColorRow,{colors:COLORS,current:ac2,onPick:function(v){setAc2(v);s.accent2=v;refreshUser();setTimeout(refreshProfile,100)}}),
h(FormInput,{title:"",placeholder:"e.g. #0000FF",value:ac2,onChange:function(v){setAc2(v);s.accent2=v;refreshUser();setTimeout(refreshProfile,100)}})),
h(FormSection,{title:"Media"},
h(RN.TouchableOpacity,{style:{backgroundColor:"#5865F2",padding:12,borderRadius:8,marginBottom:8,alignItems:"center"},onPress:function(){pickImage(function(uri){setAv(uri);s.avatar=uri;refreshUser()})}},h(RN.Text,{style:{color:"#fff",fontWeight:"600"}},"Pick Avatar")),
av?h(RN.Image,{source:{uri:av},style:{width:64,height:64,borderRadius:32,marginBottom:8}}):null,
h(FormInput,{title:"Avatar URL",placeholder:"https://i.imgur.com/...png",value:av,onChange:function(v){setAv(v);s.avatar=v;refreshUser()}}),
h(RN.TouchableOpacity,{style:{backgroundColor:"#5865F2",padding:12,borderRadius:8,marginBottom:8,alignItems:"center"},onPress:function(){pickImage(function(uri){setBa(uri);s.banner=uri;refreshUser()})}},h(RN.Text,{style:{color:"#fff",fontWeight:"600"}},"Pick Banner")),
ba?h(RN.Image,{source:{uri:ba},style:{width:"100%",height:96,borderRadius:8,marginBottom:8}}):null,
h(FormInput,{title:"Banner URL",placeholder:"https://i.imgur.com/...png",value:ba,onChange:function(v){setBa(v);s.banner=v;refreshUser()}}),
h(RN.TouchableOpacity,{style:{backgroundColor:"#5865F2",padding:12,borderRadius:8,marginBottom:8,alignItems:"center"},onPress:function(){pickImage(function(uri){setAd(uri);s.avatarDecoration=uri;refreshUser()})}},h(RN.Text,{style:{color:"#fff",fontWeight:"600"}},"Pick Decoration")),
ad?h(RN.Image,{source:{uri:ad},style:{width:64,height:64,borderRadius:32,marginBottom:8}}):null,
h(FormInput,{title:"Decoration URL",placeholder:"https://cdn.discord...",value:ad,onChange:function(v){setAd(v);s.avatarDecoration=v;refreshUser()}})),
...
if(IU){
const oa=IU.getUserAvatarURL
if(oa){
IU.getUserAvatarURL=function(){
const a=arguments,f=a[0],id=f&&typeof f==="object"?f.id:f,cu=UserStore.getCurrentUser()
if(s.enabled&&cu&&id===cu.id&&s.avatar)return s.avatar
return oa.apply(this,arguments)
}
patches.push(function(){IU.getUserAvatarURL=oa})
}
const ob=IU.getUserBannerURL
if(ob){
IU.getUserBannerURL=function(){
const a=arguments,f=a[0],id=f&&typeof f==="object"?f.id:f,cu=UserStore.getCurrentUser()
if(s.enabled&&cu&&id===cu.id&&s.banner)return s.banner
return ob.apply(this,arguments)
}
patches.push(function(){IU.getUserBannerURL=ob})
}
}
let IM;try{IM=findByProps("getAvatarURL","getDefaultAvatarURL")}catch(e){}
if(IM&&IM.getAvatarURL){
const og=IM.getAvatarURL
IM.getAvatarURL=function(){
const a=arguments,id=a[0],cu=UserStore.getCurrentUser()
if(s.enabled&&cu&&id===cu.id&&s.avatar)return s.avatar
return og.apply(this,arguments)
}
patches.push(function(){IM.getAvatarURL=og})
}
let AS;try{AS=findByProps("getUserAvatarSource","getUserBannerSource")}catch(e){}
if(AS){
if(AS.getUserAvatarSource){
const oa2=AS.getUserAvatarSource
AS.getUserAvatarSource=function(){
const a=arguments,f=a[0],id=f&&typeof f==="object"?f.id:f,cu=UserStore.getCurrentUser()
if(s.enabled&&cu&&id===cu.id&&s.avatar)return{uri:s.avatar}
return oa2.apply(this,arguments)
}
patches.push(function(){AS.getUserAvatarSource=oa2})
}
if(AS.getUserBannerSource){
const ob2=AS.getUserBannerSource
AS.getUserBannerSource=function(){
const a=arguments,f=a[0],id=f&&typeof f==="object"?f.id:f,cu=UserStore.getCurrentUser()
if(s.enabled&&cu&&id===cu.id&&s.banner)return{uri:s.banner}
return ob2.apply(this,arguments)
}
patches.push(function(){AS.getUserBannerSource=ob2})
}
}
const UPS=findByProps("getUserProfile","getGuildMemberProfile")
...
const od=Dec.getAvatarDecorationURL
Dec.getAvatarDecorationURL=function(){
if(s.enabled&&s.avatarDecoration)return s.avatarDecoration
return od.apply(this,arguments)
}
patches.push(function(){Dec.getAvatarDecorationURL=od})
}
refreshUser()
